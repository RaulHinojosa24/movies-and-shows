...
export default function GeneralMedia ({ title, backdrops = [], posters, pageType, className = '' }) {
    ...
    <>
      <Section title='Multimedia' className={'space-y-4 ' + className}>
        <div className='relative flex flex-row rounded overflow-hidden'>
          <div className='w-full flex flex-col sm:flex-row md:flex-col lg:flex-row'>
            {wideEls.length > 0 &&
              wideEls.map(el =>
                <Image key={el.file_path} pageType={pageType} element={el} title={title} mediaType='backdrop' className='w-full sm:w-1/2 md:w-full lg:w-1/2' />)}
            {wideEls.length <= 1 && highEls.length > 0 &&
              <div className='flex w-full sm:w-1/2 md:w-full lg:w-1/2'>
                {highEls.slice(0, 3).map(el =>
                  <Image key={el.file_path} pageType={pageType} element={el} title={title} mediaType='poster' className='w-1/3' />)}
              </div>}
          ...
          {wideEls.length === 2 && highEls[0] &&
            <div className='shrink-0 w-[42.86%] sm:w-[15.8%] md:w-[42.86%] lg:w-[15.8%]'>
              <Image key={highEls[0].file_path} pageType={pageType} element={highEls[0]} title={title} mediaType='poster' />
            </div>}
