...
export default function WatchProvidersFilter ({ watchProvidersPromise, watchProviders, setWatchProviders }) {
  ...
  return (
    <FilterItem title='Proveedores'>
      <Suspense fallback='Cargando proveedores...'>
        <Await resolve={Promise.all([watchProvidersPromise, config])}>
          {(promiseData) => {
            const [loadedWatchProviders, loadedConfig] = promiseData
