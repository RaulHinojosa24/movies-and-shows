...
export default function Pagination ({ totalPages }) {
  ...
  return (
    <ol className='w-fit mx-auto flex gap-2'>
      ...
      {pages.map(page => {
        const activeClass = page === currentPage ? 'font-bold dark:bg-neutral-800 bg-neutral-200' : ''

        return (
          <li key={page}>
            <button className={'rounded border-1 px-2 h-full ' + activeClass} onClick={() => updatePage(page)}>{page}</button>
          </li>
        )
      })}
      <li>
